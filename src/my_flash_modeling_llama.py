# coding=utf-8
# Modification Copyright 2024 Tong Wu
...
class LlamaRotaryEmbedding(torch.nn.Module):
    def __init__(self, dim, max_position_embeddings=2048, base=10000, device=None, enable_head_metrics=False):
        super().__init__()
        ...
        self._set_cos_sin_cache(
            ...
            dtype=torch.get_default_dtype(), enable_head_metrics=enable_head_metrics
        )

    def _set_cos_sin_cache(self, seq_len, device, dtype, enable_head_metrics=False):
    ...
    def forward(self, x, seq_len=None, enable_head_metrics=False):
        # x: [bs, num_attention_heads, seq_len, head_size]
        if seq_len > self.max_seq_len_cached:
            self._set_cos_sin_cache(seq_len=seq_len, device=x.device, dtype=x.dtype, enable_head_metrics=enable_head_metrics)
...
class LlamaLinearScalingRotaryEmbedding(LlamaRotaryEmbedding):
    ...
    def __init__(self, dim, max_position_embeddings=2048, base=10000, device=None, scaling_factor=1.0, enable_head_metrics=False):
        self.scaling_factor = scaling_factor
        super().__init__(dim, max_position_embeddings, base, device, enable_head_metrics)
    ...
    def _set_cos_sin_cache(self, seq_len, device, dtype, enable_head_metrics=False):
        ...
        if enable_head_metrics:
            ms_scale = torch.arange(1, 2, 1/32).unsqueeze(1).unsqueeze(2)  # [num_heads, 1, 1]
            emb_cos = torch.mul(ms_scale, emb.cos().repeat(32, 1, 1))  # [num_heads, seq_len, dim]
            emb_sin = torch.mul(ms_scale, emb.sin().repeat(32, 1, 1))
            self.register_buffer("cos_cached", emb_cos[None, :, :, :].to(dtype), persistent=False)  # [1, num_heads, seq_len, dim]
            self.register_buffer("sin_cached", emb_sin[None, :, :, :].to(dtype), persistent=False)

        else:
            self.register_buffer("cos_cached", emb.cos()[None, None, :, :].to(dtype), persistent=False)  # [1, 1, seq_len, dim]
            self.register_buffer("sin_cached", emb.sin()[None, None, :, :].to(dtype), persistent=False)
...
class LlamaYaRNScaledRotaryEmbedding(torch.nn.Module):
    def __init__(self, dim, max_position_embeddings=2048, base=10000, scale=1, original_max_position_embeddings=2048, extrapolation_factor=1, attn_factor=1, beta_fast=32, beta_slow=1, device=None):
        super().__init__()
...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids):
    if cos.size(0) != 1 and cos.size(1) != 1:
        # [bs, num_heads, seq_len, dim] -> [bs, seq_len, num_heads, dim]
        cos = torch.gather(
            cos, 2, position_ids.unsqueeze(1).unsqueeze(-1).expand(-1, cos.size(1), -1, cos.size(-1))
        ).transpose(1, 2)
        sin = torch.gather(
            sin, 2, position_ids.unsqueeze(1).unsqueeze(-1).expand(-1, sin.size(1), -1, sin.size(-1))
        ).transpose(1, 2)
    elif cos.size(0) == 1 and cos.size(1) != 1:
        # [num_heads, seq_len, dim] -> [bs, seq_len, num_heads, dim]
        cos = cos.squeeze().transpose(0, 1)[position_ids]
        sin = sin.squeeze().transpose(0, 1)[position_ids]
    else:
        cos = cos.squeeze()
        sin = sin.squeeze()
        # [bs, seq_len, dim] -> [bs, seq_len, 1, dim]
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.rope_theta = 10000.0  # config.rope_theta

        self.enable_head_metrics = False  # False
        ...
        if self.config.rope_scaling is None:
            ...
            if scaling_type == "linear":
                self.rotary_emb = LlamaLinearScalingRotaryEmbedding(
                    self.head_dim, max_position_embeddings=self.max_position_embeddings, scaling_factor=scaling_factor, enable_head_metrics=self.enable_head_metrics
                )
    ...
    def _head_wise_statistics(self, query_states, key_states, q_len, kv_seq_len, bsz, attention_mask):
        ...
        head_orders = torch.zeros([bsz, self.num_heads])
        chunks = torch.chunk(attn_weights, chunks=attn_weights.size(0), dim=0)

        for i, chunk in enumerate(chunks):
            chunk = chunk.squeeze(0)
            assert chunk.size() == (self.num_heads, q_len, kv_seq_len)
            head_orders[i] = self._sort_by_score(chunk)

        return head_orders.to(torch.int)
    
    def _sort_by_score(self, attn_weights):
        # attn_weights: [num_heads, q_len, kv_seq_len]
        lower_triangular = torch.tril(attn_weights.to(torch.float16), diagonal=-32)  # 2.0.1 -> tril BF16 Error
        sum_lower_triangular = lower_triangular.sum(dim=(1, 2))
        head_orders = torch.argsort(-sum_lower_triangular)
    ...
    def forward(
        ...
        if self.enable_head_metrics:
            ...
            new_k = k
            if has_layer_past:
                kv_seq_len = past_len + q.size(1)
                kv = torch.stack([k, v], 2)
                kv = repeat_kv(kv, self.num_key_value_groups)
                kv = torch.cat([past_kv[:, :past_len], kv], 1)
                new_k = kv[:, :, 0, :, :]

            head_order = self._head_wise_statistics(q, new_k, q_len, kv_seq_len, bsz, attention_mask)

            cos, sin = self.rotary_emb(v, seq_len=past_len, enable_head_metrics=self.enable_head_metrics)
            cos = cos.squeeze()[head_order, :, :]
            sin = sin.squeeze()[head_order, :, :]

        else:
            cos, sin = self.rotary_emb(v, seq_len=past_len)
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        # when is_padded_inputs=True, it will out of memory
        # if attention_mask is not None:
